...
Ihr könnt auch eine `GitHub-Aktion <https://github.com/features/actions>`_
erstellen, die ein Paket erstellt und auf PyPI hochlädt. Eine solche
:file:`.github/workflows/pypi.yml`-Datei könnte folgendermaßen aussehen:
...
Zeile 36
    Die GitHub-Aktion `actions/download-artifact
    <https://github.com/actions/download-artifact>`_ stellt die gebauten
    Verteilungspakete bereit.
Zeile 38–41
    Die GitHub-Aktion `pypa/gh-action-pypi-publish
    <https://github.com/pypa/gh-action-pypi-publish>`_ veröffentlicht die Pakete
...
Vermeidet unsichere Trigger
    ...
    `pull_request_target <https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#pull_request_target>`_ sollte
    vermieden werden.
...
Säubert Parameter und Eingaben
    ...
    übergebt, um `Template-Injection
    <https://docs.zizmor.sh/audits/#template-injection>`_-Angriffe zu vermeiden.
...
CI/CD-Konfigurationen von GitHub Actions aufspüren und beheben, wie :abbr:`z. B.
(zum Beispiel)` :ref:`token_exfiltration`. zizmor ist für die Integration in
GitHub Actions konzipiert. Eine typische GitHub Action von uns für zizmor sieht
so aus:
...
Ihr könnt zizmor jedoch auch als pre-commit-Check verwenden, um Probleme zu
erkennen, bevor sie gepusht werden:
...
zizmor verfügt außerdem über eine `VS-Code-Integration
<https://marketplace.visualstudio.com/items?itemName=zizmor.zizmor-vscode>`_ und
kann als LSP-Server (``zizmor --lsp``) ausgeführt werden, um in jedem Editor
